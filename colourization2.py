...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        ...
        #(64L, 64L, 3L, 3L)
        new_length=(self.kernel_size+2*self.dilation)
        scale=new_length//self.kernel_size
        if(scale!=1):
            #scale=2
            weight1=torch.FloatTensor(F.upsample(input=self.weight, 
                scale_factor=scale,mode='nearest').data)
            weight2=torch.FloatTensor(F.upsample(input=self.weight, 
                scale_factor=scale,mode='bilinear').data)
            #torch.FloatTensor( Variable(torch.eq(weight1,weight2)))
            temp=torch.autograd.Variable(torch.eq(weight1,weight2))
            print(temp.data.numpy())
            temp=torch.FloatTensor(temp.data.numpy())
            print(temp)
            weight=torch.autograd.Variable(torch.FloatTensor(F.upsample(input=self.weight, 
                scale_factor=scale,mode='nearest').data)*temp)
        else:
            weight=self.weight
